...
Human artifact PATCH and model execution share the task-level admission check under the project lock. Once execution exists, only chapter tasks in the current pending extension range may be edited or generated; consumed chapters and upstream planning return `403 opening_consumed_planning_locked` before any mutation. Accepted future chapter artifacts can still be edited through validation/revision checks. Publication retains its independent consumed-revision, old-outline and old-handoff checks. The regression exercises valid rejected edits to story core, chapter 1 and chapter 50, checks the entire project file tree is unchanged, then completes failure/conflict/release retries with an accepted human edit to future chapter 60. Non-Opening editing keeps its existing behavior.
