...
def test_web_game_review_marks_only_bystander_remaining_after_allocation_as_missing():
    review = review_web_game_chapter(
        chapter_number=4,
        body="夜烬把五点加到智力上。随后他确认加点。短发玩家的可用属性点还剩4点。",
        event_plan={
            "novel_type": "game_webnovel",
...
def test_reviewer_agent_forwards_known_character_names_to_web_game_evidence():
    review = review_reviewer_agent(
        chapter_number=4,
        body="夜烬把五点加到智力上。林峰看了夜烬一眼，他确认加点，提示消失后，他的面板上的可用属性点还剩4点。",
        event_plan={"novel_type": "game_webnovel", "attribute_allocation_decision": {"mode": "allocate", "allocations": {"智力": 5}, "remaining": 4}},
        world_facts=[],
        protagonist_names=["夜烬"],
...
def test_web_game_review_rejects_conditional_action_without_explicit_turn():
    review = review_web_game_chapter(
        chapter_number=4,
...
def test_web_game_review_rejects_conditional_action_with_actuality_word():
    review = review_web_game_chapter(
        chapter_number=4,
