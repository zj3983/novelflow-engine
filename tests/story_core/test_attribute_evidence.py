...
def test_confirmed_allocation_accepts_a_protagonist_alias_inside_a_panel_owner_prefix():
    body = "夜烬把五点加到智力上。随后他确认加点。此时夜烬的面板上的可用属性点归零。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) == 0


def test_confirmed_allocation_rejects_a_pronoun_panel_owned_by_a_nearer_bystander_subject():
    body = "夜烬把五点加到智力上。随后他确认加点。短发玩家确认完，他的面板上的可用属性点还剩四点。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) is None


def test_confirmed_allocation_accepts_a_pronoun_panel_owned_by_the_nearer_protagonist_subject():
    body = "夜烬把五点加到智力上。夜烬确认完，他的面板上的可用属性点归零。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) == 0


def test_confirmed_allocation_rejects_a_pronoun_panel_after_a_bystander_opens_it():
    body = "夜烬把五点加到智力上。随后他确认加点。短发玩家打开面板后，他的面板上的可用属性点还剩四点。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) is None


def test_confirmed_allocation_accepts_a_pronoun_panel_after_the_protagonist_opens_it():
    body = "夜烬把五点加到智力上。夜烬确认加点，夜烬打开面板后，他的面板上的可用属性点归零。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) == 0


def test_confirmed_allocation_accepts_a_sentence_initial_pronoun_panel_near_the_protagonist_anchor():
    body = "夜烬把五点加到智力上。随后他确认加点。他的面板上的可用属性点归零。"
...
def test_confirmed_allocation_rejects_a_pronoun_panel_after_a_bystander_and_status_clause():
    body = "夜烬把五点加到智力上。短发玩家点下确认，提示消失后，他的面板上的可用属性点还剩四点。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) is None


def test_confirmed_allocation_accepts_a_pronoun_panel_after_the_protagonist_and_status_clause():
    body = "夜烬把五点加到智力上。夜烬点下确认，提示消失后，他的面板上的可用属性点归零。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) == 0


def test_confirmed_allocation_accepts_a_pronoun_confirmation_chain_from_the_protagonist_anchor():
    body = "夜烬把五点加到智力上。他点下确认，提示消失后，他的面板上的可用属性点归零。"

    assert latest_confirmed_attribute_points(body, protagonist_aliases={"夜烬"}) == 0


def test_confirmed_allocation_rejects_a_pronoun_confirmation_chain_after_a_bystander_anchor():
    body = "夜烬把五点加到智力上。短发玩家点下确认，提示消失后，他的面板上的可用属性点还剩四点。"
