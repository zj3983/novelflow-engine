...
def test_web_game_review_rejects_first_chapter_missing_class_choice_and_panel():
    body = (
    ...
    review = review_web_game_chapter(
        ...
        world_facts=["职业规则：第一章必须写出职业选择，并在角色面板里记录职业。"],
    )

    assert review["pass"] is False
    assert any("职业选择" in issue for issue in review["issues"])
    assert any("角色面板" in issue for issue in review["issues"])


def test_web_game_review_allows_first_chapter_class_choice_panel_and_single_npc():
    body = (
        "《天启之门》开服当晚，苏叶在出租屋里完成登录，游戏ID夜烬进入角色创建界面。"
        "职业选择栏在战士、游侠、牧师和法师之间轮转，他最终选择元素法师学徒。"
        "【角色面板】游戏ID：夜烬；等级：1；职业：元素法师学徒；经验：0/100；"
...
def test_opening_review_requires_class_choice_and_panel():
    body = (
...
def test_web_game_prompt_rules_include_class_choice_and_panel():
    rules = "\n".join(web_game_review_rules())

    assert "职业选择" in rules
