...
def test_xuanhuan_context_keeps_only_general_and_xuanhuan_examples(monkeypatch) -> None:
    ...
    assert "玄幻正例" in instructions
    assert "石碑只认完整拳路" in instructions
...
def test_every_module_keeps_complete_examples_and_required_sections(monkeypatch) -> None:
    ...
    assert "掌柜把验货单推回来，说：“印章是真的，但担保人已经失踪，你得先找到他。”" in instructions
    assert "掌柜把验货单推回来，说：“你先回去吧，这件事以后再说。”林峥问：“那我下一步该做什么？”掌柜只说：“事情没那么简单。”" in instructions
    assert "正例有效，因为反馈明确，阻碍由当前验货行动触发，下一步也能直接落到寻找担保人。" in instructions
    assert "反例失败，因为拒绝没有具体动机，结尾只剩模糊判断，也没有可执行的问题。" in instructions
