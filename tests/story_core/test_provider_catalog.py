...
def test_presets_have_complete_metadata_and_model_suggestions():
    for provider_id in BUILTIN_PROVIDER_IDS:
        ...
        if provider_id not in {"codexcli", "antigravity", "custom_openai"}:
...
def test_gemini_preset_exposes_current_models_instead_of_retired_2_5_models():
    definition = provider_definition("gemini")
    ...
    assert all("gemini-2.5" not in model for model in definition.planner_models)
    assert all("gemini-2.5" not in model for model in definition.writer_models)
