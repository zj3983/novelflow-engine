...
def test_chapter_seed_carries_outline_hard_constraints() -> None:
    story = StoryState(
        story_id="s-outline-contract",
        outline="夜烬在神域开始新手任务。",
        genre="网游",
        style="自然口语",
        outline_context={
            "chapter": {
                "chapter_number": 1,
                "goal": "完成第一次异常验证",
                "must_include": ["灰狼毒腺进度为8/16", "使用新手法杖"],
    ...
    assert seed["must_show"] == ["灰狼毒腺进度为8/16", "使用新手法杖"]
    assert seed["must_not_write"] == ["不要提交清道夫委托"]
    assert summary["正文要露出"] == ["灰狼毒腺进度为8/16", "使用新手法杖"]
    assert summary["正文别写"] == ["不要提交清道夫委托"]
