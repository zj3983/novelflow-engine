...
def _phase(chapter_number: int) -> str:
    ...
    if chapter_number == 2:
        return "黄金三章第2章：用千倍爆率完成新手任务/装备门槛，制造第一次明确领先。"
    if chapter_number == 3:
    ...
    if chapter_number == 1:
        return {
            "required_beats": [
                ...
                "登录或建号阶段必须写出游戏ID/网名。",
                "必须写出职业选择，并让职业进入角色面板。",
                "必须出现短角色面板：ID、等级、职业、经验、生命/法力、主武器或基础技能、货币/背包关键项；不要展开易漂移的扩展属性。",
                "金手指必须先有触发伏笔，再完成首次验证，让读者看到爆率优势会转化为任务、装备、技能或路线门槛上的提前一步。",
                ...
                "本章结尾必须给出未闭合账本：经验、毒腺/狼皮库存、0铜、装备耐久、任务未提交状态，并指出下一步能比普通玩家少跑几趟或提前摸到哪个入口。",
            ],
            "forbidden_moves": [
                ...
                "禁止第一章实际提交清道夫委托、领取30铜、扣费修理装备、购买药水或把材料处理成服务闭环。",
            ...
            "world_reaction_targets": [
                ...
                "章末只留下下一章任务、装备、技能或路线门槛目标，不让外部势力正式介入。",
            ],
        }
    if chapter_number == 2:
        return {
            "required_beats": [
                ...
                "通过千倍爆率更快补齐任务、装备、技能或路线门槛，让主角相对普通玩家明确领先。",
