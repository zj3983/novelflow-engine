    ...
    if chapter_number == 1:
        return {
            "required_beats": [
                ...
                "登录或建号阶段必须写出游戏ID/网名。",
                "必须写出职业选择，并让职业进入角色面板。",
                "必须出现短角色面板：ID、等级、职业、经验、生命/法力、主武器或基础技能、背包/钱袋关键项；不要写成“货币：0铜”，不要展开易漂移的扩展属性。",
...
def _newbie_scene_plan(chapter_number: int) -> list[dict[str, str]]:
    ...
    if chapter_number == 2:
        return [
            ...
            {"key": "service_nodes", "goal": "修杖、买药，写清铜币、耐久、药水冷却"},
...
def _game_genre_craft(chapter_number: int, stage: str) -> dict[str, Any]:
    ...
    return {
        ...
        "dialogue_method": [
            ...
            "每章至少写一轮连续问答：对方先说一句带原因或抱怨的话，主角接一句正常解释或拒绝，再由对方给出反应；不要只写口令式短句。",
...
def _game_satisfaction_loop(chapter_number: int, stage: str) -> dict[str, str]:
    if chapter_number == 1:
        return {
            ...
            "visible_payoff": "至少兑现一项账本变化或下一步前置条件；交任务、拿铜币、修杖、买药必须由项目账本/章节计划允许，未允许时只写材料、损耗和前置条件。",
    ...
    if stage == "newbie_low":
        return {
            ...
            "core_event": "围绕一个低级目标推进：补材料、交任务、修杖、买书、试入口或完成基础命中记录。",
...
def _writing_contract_for_game(story: StoryState, chapter_number: int) -> dict[str, Any]:
    ...
    if stage == "newbie_low":
        ...
        return {
            ...
            "allowed_progress": [
                "登录/建号和职业选择" if chapter_number == 1 else "新手村任务、低级地图和基础材料账本",
