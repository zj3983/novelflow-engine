...
GAME_WEBNOVEL = GenrePlugin(
    ...
    core_promises=(
        "读者期待看到稳定的等级、经验、掉落、装备、交易和势力压迫循环。",
        "隐藏优势可以很强，但必须伴随身份暴露、市场异常、公会追踪或资源消化风险。",
    ),
    ledger_fields=("等级", "经验", "金币", "装备", "技能", "任务", "公会关注度", "金手指暴露度", "市场异常度"),
    rulebook={
        "progression_rules": (
        ...
        "chapter_formula": (
            ...
            "每章都要同时推进主角成长、世界规则展示和外部势力反应，避免只写打怪流水账。",
            "网游第一章只聚焦一个核心事件：现实压力、登录/建号、选择游戏ID、选择职业、角色面板、首次小额验证、章末下一步目标。",
    ...
    return {
        ...
        "opening_scene_templates": [
            ...
            },
            {
                ...
                "location": "灰烬村外",
                "purpose": "用低级怪物或低级材料小额验证千倍爆率。",
                "conflict": "收益让主角确认优势，但规模不足以惊动全服。",
                "must_show": ["低级怪物", "掉落反馈", "背包变化", "小额验证"],
            },
            {
                "id": "single_npc_service",
                "location": "灰烬村",
                "purpose": "只完整展开一个命名NPC服务节点。",
                "conflict": "NPC只能在岗位范围内提供服务、报价、任务或门槛。",
                "must_show": ["NPC地点", "服务内容", "价格/门槛", "信息边界"],
            },
            {
                "id": "chapter_1_next_step",
                "location": "灰烬村内",
                "purpose": "只作为章末下一步目标，不展开实际寄售、成交、到账或商人盘点。",
                "conflict": "下一步目标已经出现：主角知道材料后续可以处理，但第一章只收在规则未明前的谨慎决定。",
                "must_show": ["下一步目标", "材料暂不外露", "NPC门槛", "规则未明"],
            },
        ],
        "conflict_ladders": {
            "chapter_1": ["现实压力", "职业选择", "小额验证", "NPC门槛", "下一步目标"],
            "chapter_2": ["路线成本", "补给/耐久", "NPC服务限制", "重复批次弱异常", "公会外围试探"],
            "chapter_3": ["资源点秩序", "压价/拉拢", "职业试炼门槛", "论坛传闻", "小高潮"],
        },
