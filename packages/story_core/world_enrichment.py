    ...
    if "game_webnovel" in plugin_ids:
        return {
            ...
            "reaction_rules": [
                ...
                "公会不会立刻知道真相；只有重复出货模式、稀有物品、榜单公告、资源点目击、NPC任务异常或系统风控多源汇总后，才会逐步缩小范围。",
...
def _default_world_systems(project: NovelProject, genre_plugins: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if "game_webnovel" in plugin_ids:
        return {
            ...
            "conflict_engines": [
                "千倍爆率带来收益，同时制造市场异常和身份暴露风险。",
                "主角越想低调变现，就越需要拆单、绕路、压价或付出时间成本。",
                "现实兑换渠道尚未稳定时，主角的收益先体现为游戏内议价权、材料储备和商人关注，而不是立刻折算成现实暴富。",
                "10级职业试炼临近后，攻略垄断、材料囤积和队伍名额会自然升级为阶段冲突。",
            ],
            "causal_loops": [
                {"name": "材料涌入", "description": "主角高频低级材料出货 -> 交易行价格波动 -> 商人记录时间戳和批次 -> 多次重复后才形成可疑货源画像，公会不能单次精准定位。"},
...
def _default_server_runtime(project: NovelProject, genre_plugins: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "anti_cheat_rules": [
            "连续异常掉落、异常寄售和不合理升级速度会提高系统风控或玩家举报概率。",
            ...
            "golden_three_chapters": {
                "chapter_1": {
                    ...
                    "must_include": [
                        ...
                        "混沌之种/千倍爆率的首次验证，必须同时展示收益和风控代价。",
                        "交易行基础机制：匿名寄售、手续费、流水记录、异常标记和商人盯盘。",
                ...
                "chapter_2": {
                    ...
                    "conflict_modes": [
                        "资源路线压力：刷怪点拥挤、补给消耗、装备耐久、背包容量和路线成本限制主角效率。",
                    ...
                    "must_include": [
                        ...
                        "交易行价格波动与商人玩家追踪模型进一步成形，但必须遵守弱线索递进，不能单次精准定位。",
    ...
    if _has_game_plugin(genre_plugins):
        return {
            ...
            "phase_beats": [
                ...
                {"range": "4-8", "purpose": "围绕西林狼坡和黑水沼泽扩大收益，补足NPC任务、补给成本、耐久和市场批次异常。"},
...
def _default_longform_framework(project: NovelProject, genre_plugins: list[dict[str, Any]]) -> dict[str, Any]:
    if _has_game_plugin(genre_plugins):
        return {
            ...
            "volume_ladder": [
                {"range": "1-30", "title": "灰烬村蛰伏", "unlock": "1-10级、千倍爆率小额验证、第一条隐蔽变现渠道", "pressure_cap": "商人盯盘和公会外围弱试探"},
            ...
            "faction_ladder": [
                ...
                "官方运营/系统风控：现实资本卷之前只做规则与风控，不直接下场解释真相。",
                "现实资本：中后期通过账号合同、黑市和线下身份风险介入。",
                "世界权限持有者：终局阶段才揭示为真正对手。"
            ],
            "economy_ladder": [
                "第1卷：铜币/银币、低级材料、匿名寄售、手续费、市场弱异常；不能现实暴富。",
    ...
    return [
        {
            ...
            "speech_style": "短句，先确认规则和成本，再做决定。",
            "goals": ["安全升到10级", "建立隐蔽变现渠道", "隐藏混沌之种"],
            "secrets": ["混沌之种", "千倍爆率"],
            "conflict_hooks": ["交易行批次异常", "职业试炼门槛", "公会外围试探"],
        },
        {
            "name": "赵胖子",
            "game_id": "铁算盘",
            ...
            "conflict_hooks": ["压价试探", "供货关系", "交易行风控"],
        },
        {
            "name": "药剂师洛婶",
            "game_id": "药剂师洛婶",
