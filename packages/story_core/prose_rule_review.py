...
PROMPT_CRAFT_GUARDS = [
    "段首写法：动作、物件、对话、环境反馈轮流起段，主语自然变化。",
    "段落写法：长短段交替；关键动作和对话用4-6句连成一块，单句段留给章末、转折或强爆点。",
    "判断句写法：少用“这是/这不是/这就是”，改用动作、物件和旁人反应让读者自己明白。",
    "比喻写法：优先写动作、温度、声音、重量和触感；比喻只留给真正需要加重的一两处。",
    "NPC对话写法：一次只漏一两件事，复杂规矩拆成两三轮问答，让价格、岗位、前置条件从接话里露出来。",
    "主角发声：至少主动开口一次，询价、拒绝或试探都可以；他说完整理由，再让对方接一句反应。",
    "信息边界：外部消息落在价格、队伍、公告、对话、物件和环境变化上，主角只处理自己周围可见的信息。",
    "战斗写法：用动作、距离、错位、消耗和身体反馈写清过程，数字和术语只服务当场选择。",
    "情绪写法：章首、章中、章末各给一处小动作或小念头，让不爽、疲惫、犹豫、侥幸或苦笑落到身体反应上。",
]
...
_PLANNING_META_LOCATION_SUFFIX_PATTERN = r"(?:旁边|边上|前面|后面|前方|后方|边|旁|前|后)"
_PLANNING_META_LOCATION_END_PATTERN = (
    rf"(?:(?:的\s*)?{_PLANNING_META_LOCATION_SUFFIX_PATTERN}"
    rf"(?:的|{_PLANNING_META_RIGHT_BOUNDARY_PATTERN})"
    rf"|{_PLANNING_META_RIGHT_BOUNDARY_PATTERN})"
)
_PLANNING_META_ACTION_CONTINUATION_PATTERN = r"(?:之后|以前|的时候|时|后(?!续)|前)"
