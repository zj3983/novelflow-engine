...
def anti_ai_style_rules() -> list[str]:
    ...
    return [
        "拒绝华丽辞藻堆砌、拒绝成语套话、拒绝书面生硬表达、拒绝流水账、不要模板化心理描写、用词口语化生活化、短句为主、人物行为符合人设、前后逻辑严谨、不要重复句式。",
...
def review_prose_style(body: str) -> dict[str, Any]:
    ...
    if stiff_dialogue:
        ...
        _append_issue(
            ...
            plan="按角色身份改写对白：短句、口语、带动作；莽夫别文绉绉，商人要算账，NPC要有岗位口吻。",
            score=6,
        )
    ...
    if short_ratio >= 0.5 or (short_ratio >= 0.38 and len(mechanical_terms) >= 2) or len(mechanical_terms) >= 6:
        ...
        _append_issue(
            ...
            plan="合并连续短句，删掉“意味着/很直接/很清楚/不能/需要”等报告式判断；把规则变成动作、对话、犹豫、环境反馈和具体代价。",
            score=5,
        )
