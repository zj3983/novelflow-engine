...
function narrativeFunctionOf(character: DisplayCharacter): string {
  const explicit = enumToken(character.narrative_function);
  if (explicit) return explicit;
  const legacy = enumToken(character.role || character.chapter_role || character.character_tier);
  return legacy;
}

function isProtagonist(character: DisplayCharacter): boolean {
  if (enumToken(character.narrative_function)) return narrativeFunctionOf(character) === "protagonist";
  return narrativeFunctionOf(character) === "protagonist" || enumToken(character.character_tier) === "protagonist";
}
