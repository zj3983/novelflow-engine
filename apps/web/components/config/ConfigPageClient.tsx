...
function validateProviderAccount(
  ...
  if (provider.protocol === "codex_cli" && !account.codex_command.trim()) return "Codex CLI 命令不能为空";
  if (provider.protocol !== "codex_cli" && !isHttpUrl(account.base_url)) return `${provider.name} API 地址格式不正确`;
  return "";
}
...
export function ConfigPageClient() {
  ...
  async function testProvider(providerId: string) {
    ...
    const account = settings.accounts[providerId] ?? (provider ? { api_key: "", base_url: provider.default_base_url, custom_models: [], codex_command: provider.protocol === "codex_cli" ? "codex" : "" } : null);
    if (!provider || !account) return;
    const currentStage = (["planner", "writer"] as const).find((stage) => settings.stages[stage].provider_id === providerId) ?? "planner";
    const model = settings.stages[currentStage].provider_id === providerId
  ...
  return (
    <main className="config-shell" aria-busy={busy}>
      <div className="config-shell__primary">
        <ProviderAccountsCard value={settings} providers={providers} statuses={connections} disabled={busy} onChange={setSettings} onTest={(providerId) => void testProvider(providerId)} />
